    ...
    <Swiper
      ...
      {sliderContent.map((slider, i) => (
        <SwiperSlide
          ...
          className={`bg-no-repeat sm:bg-center ${
            i === 0 ? "bg-center sm:bg-right" : ""
          } ${i === 2 ? "bg-left-bottom" : ""}   bg-cover flex`}>
          <GlobalContainer addClassName=" flex flex-col justify-center">
            <Text
              textType={TextType.Title}
              addClassName="text-white max-w-[685px] font-extrabold">
              {slider.title}
            </Text>
            <Text
              textType={TextType.Regular}
              addClassName="text-white max-w-[540px]">
              {slider.subTitle}
            </Text>
          </GlobalContainer>
        </SwiperSlide>
